...
function start(){
    ...
    if(new Date(Info.nowTime)<new Date('2018/04/20 22:59:59')){
        ...
        if(new Date(Info.nowTime)>new Date('2018/04/17 23:59:59')){
            ...
            var todayN = ntime + ' 22:59:59';

            if(new Date(Info.nowTime)>new Date(todayS)){
                if(new Date(Info.nowTime)>new Date(todayE)){
                    if(new Date(Info.nowTime)>new Date(todayN)){
                        var d = addDate(ntime,1);
                        var endT = d + ' 10:00:00';
                        app.part3time = true;
                        cutTimeEnd(backTime,endT);
        ...
        success: function(res) {
            ...
            switch(res.RspCode){
                case 200:
                    ...
                    // app.suc.title = 'images/sm_2.png';
                    switch(app.p3Index){
                        case 0:
                            app.suc={
                                title:'images/sm_2.png',
                                img:'images/sv_1.png',
                                btn_1:'立即领取 ',//国内
                                btn_2:''
                            }
                ...
                case 501:
                    app.fail={
                        img:'images/seach.png',
                        text:'抱歉，手慢啦，<br>大奖已经被其他小伙伴抢完啦，<br>下次再来吧~',
                        btn:'知道啦'
                    }
                    showPop('fail');
                break;
...
var rule_texts_1 = '<p>1.活动时间为：2018年4月16日00:00:00-2018年4月26日23:59:59；</p>'
                ...
                +'<p>5.在活动期间，用户如果出现违规行为（包括但不限于作弊领取、虚假交易等），xxxxxx将取消用户的活动资格，必要时xxxxxx将追究违规用户的法律责任。</p>';
var rule_texts_2 = '<p>1. 活动时间：2018年4月18日-2018年4月26日，每天10:00整点秒杀，数量有限，先到先得；</p>'
                ...
                +'<p>7. 在活动期间，用户如果出现违规行为（包括但不限于作弊领取、虚假交易等），xxxxxxx将取消用户的活动资格，必要时xxxxxxx将追究违规用户的法律责任。</p>';
